...
class BaseChamp:
    ...
    def race(self, value):
        ...
        if value not in ['Human', 'Elf', 'Dwarf', 'Hobbit', 'Orc']:
            raise ValueError('Race must be one of these: Human, Elf, Dwarf, Hobbit, Orc')
    ...
    def gender(self, value):
        ...
        if value not in ['Male', 'Female', 'Other']:
            raise ValueError('Race must be one of these: Male, Female or Other')
    ...
    def increase_stats(self, stat):
        ...
        if type(stat) is not str:
            raise TypeError('Select one of the stats Health, Attack, Defense, Magic, Speed')
